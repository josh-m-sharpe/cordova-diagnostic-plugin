...
{
    @try {
        NSString* status = [self getNotificationAuthorizationStatusAsString:[UNAuthorizationStatus notificationAuthorizationStatus]];
        NSLog(@"%@",[NSString stringWithFormat:@"Notification Authorization status is: %@", status]);
        [self sendPluginResult:[CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:status] :command];
    }
    @catch (NSException *exception) {
        [self handlePluginException:exception :command];
...
- (NSString*) getNotificationAuthorizationStatusAsString: (UNAuthorizationStatus)notificationAuthorizationStatus
{
    NSString* status;
    if(notificationAuthorizationStatus == UNAuthorizationStatusNotDetermined){
        status = @"not_determined";
    }else if(notificationAuthorizationStatus == UNAuthorizationStatusDenied){
        status = @"denied";
    }else if(notificationAuthorizationStatus == UNAuthorizationStatusAuthorized){
        status = @"authorized";
    }
    return status;
}
